def TrDiff(criteria, operator, value, value_type, result_df, tolerance=1e-6):
    try:
        if value_type == 'number':
            diff = round(result_df['Latest Reading'], 3) - round(result_df['Minimum Thickness'], 3)
        elif value_type == '%':
            diff = round((result_df['Latest Reading'] - result_df['Minimum Thickness']) / result_df['Latest Reading'] * 100, 0)
        ...
        raise RuntimeError(f"Error in TrDiff: {str(e)}")
